...
def validate_stock_symbol(symbol: str, available_symbols: List[str]) -> Dict[str, Any]:
    ...
    if symbol not in available_symbols:
        return {"valid": False, "message": f"Stock symbol '{symbol}' not found in available symbols"}
...
def validate_date_range(start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    if days_from_start > max_past_days:
        return {"valid": False, "message": f"Start date cannot be more than {max_past_days//365} years in the past"}
    
    if days_to_end > max_future_days:
        return {"valid": False, "message": f"End date cannot be more than {max_future_days} days in the future"}
...
def validate_dataframe(df: pd.DataFrame, required_columns: List[str]) -> Dict[str, Any]:
    ...
    if null_columns:
        return {"valid": False, "message": f"All values are null in columns: {', '.join(null_columns)}"}
...
def validate_ohlcv_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        ...
        if not invalid_hl.empty:
            return {"valid": False, "message": f"High price is less than Low price in {len(invalid_hl)} rows"}
        ...
        if not invalid_open.empty:
            return {"valid": False, "message": f"Open price is outside High-Low range in {len(invalid_open)} rows"}
        
        invalid_close = df[(df["Close"] > df["High"]) | (df["Close"] < df["Low"])]
        if not invalid_close.empty:
            return {"valid": False, "message": f"Close price is outside High-Low range in {len(invalid_close)} rows"}
        
        # Check for negative values
        price_columns = ["Open", "High", "Low", "Close"]
        for col in price_columns:
            if (df[col] <= 0).any():
                return {"valid": False, "message": f"Negative or zero values found in {col} column"}
...
def validate_portfolio_symbols(symbols: List[str], available_symbols: List[str], min_symbols: int = 2) -> Dict[str, Any]:
    """
    ...
    if len(symbols) < min_symbols:
        return {"valid": False, "message": f"Portfolio must contain at least {min_symbols} symbols"}
    ...
    if invalid_symbols:
        return {"valid": False, "message": f"Invalid symbols: {', '.join(invalid_symbols)}"}
    
    return {"valid": True, "message": "Valid portfolio symbols"}


def validate_numeric_range(value: float, min_val: float, max_val: float, field_name: str) -> Dict[str, Any]:
    """
        ...
        if filter_name in validation_rules:
            ...
            if isinstance(filter_value, (tuple, list)) and len(filter_value) == 2:
                ...
                min_validation = validate_numeric_range(
                    min_val, rules["min"], rules["max"], 
                    f"{filter_name} minimum"
                )
                ...
                max_validation = validate_numeric_range(
                    max_val, rules["min"], rules["max"],
                    f"{filter_name} maximum"
                )
                ...
                if min_val >= max_val:
                    return {"valid": False, "message": f"{filter_name} minimum must be less than maximum"}
...
def validate_file_upload(uploaded_file) -> Dict[str, Any]:
    ...
    if hasattr(uploaded_file, 'name'):
        ...
        if file_extension not in allowed_extensions:
            return {"valid": False, "message": f"File type not supported. Allowed types: {', '.join(allowed_extensions)}"}
