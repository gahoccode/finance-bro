...
def transpose_financial_dataframe(df: pd.DataFrame, name: str, period: str) -> pd.DataFrame:
    """
...
def calculate_stock_returns(prices: pd.DataFrame, return_type: str = 'simple') -> pd.Series:
    """
...
def process_portfolio_price_data(all_historical_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
...
def create_weights_dataframe(weights_dict: Dict[str, float], column_name: str) -> pd.DataFrame:
...
def format_allocation_dataframe(allocation: Dict[str, int], latest_prices_actual: pd.Series, portfolio_value: float) -> pd.DataFrame:
...
def create_performance_summary_dataframe(performance_data: List[Dict]) -> pd.DataFrame:
    ...
    for col in numeric_cols:
        if col in performance_df.columns:
            performance_df[col] = performance_df[col].apply(lambda x: f"{float(x):.4f}" if isinstance(x, (int, float)) else x)
    
    return performance_df


def prepare_portfolio_data(stock_data_dict: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def clean_financial_data(df: pd.DataFrame, remove_null_rows: bool = True) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if remove_null_rows:
            ...
            for col in df_clean.columns:
                if col.lower() in ['ticker', 'symbol', 'metric', 'index', 'yearreport', 'lengthreport']:
...
def validate_financial_dataframe(df: pd.DataFrame, required_columns: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    if required_columns:
        ...
        if missing_columns:
            return {"valid": False, "message": f"Missing columns: {', '.join(missing_columns)}"}
    
    # Check for all-null columns
    null_columns = [col for col in df.columns if df[col].isnull().all()]
    if null_columns:
        return {"valid": False, "message": f"All-null columns: {', '.join(null_columns)}"}
    
    return {"valid": True, "message": "Valid financial dataframe"}


def merge_financial_dataframes(dataframes: Dict[str, pd.DataFrame], on_column: str = 'yearReport') -> pd.DataFrame:
    """
        ...
        for name, df in dataframes.items():
            if df is not None and not df.empty and on_column in df.columns:
                if merged_df is None:
                    merged_df = df.copy()
                    merged_df.columns = [f"{name}_{col}" if col != on_column else col for col in merged_df.columns]
                else:
                    df_renamed = df.copy()
                    df_renamed.columns = [f"{name}_{col}" if col != on_column else col for col in df_renamed.columns]
...
def calculate_financial_ratios(income_statement: pd.DataFrame, balance_sheet: pd.DataFrame) -> pd.DataFrame:
    """
...
def aggregate_portfolio_metrics(returns_data: pd.DataFrame) -> Dict[str, float]:
    ...
    try:
        ...
        metrics['sharpe_ratio'] = metrics['total_return'] / metrics['volatility'] if metrics['volatility'] > 0 else 0
