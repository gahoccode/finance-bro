...
def calculate_capital_employed(BalanceSheet):
    ...
    try:
        ...
        long_term_cols = ['Long-term borrowings (Bn. VND)', 'Long-term debt (Bn. VND)', 'Long term borrowings (Bn. VND)']
        short_term_cols = ['Short-term borrowings (Bn. VND)', 'Short-term debt (Bn. VND)', 'Short term borrowings (Bn. VND)']
        equity_cols = ["OWNER'S EQUITY(Bn.VND)", "Owner's equity (Bn. VND)", "OWNER'S EQUITY", "Owner's equity", "Shareholders' equity (Bn. VND)"]
        ...
        if long_term_col is None:
            ...
            balance_sheet_copy[long_term_col] = balance_sheet_copy[long_term_col].fillna(0)

        if short_term_col is None:
            balance_sheet_copy['Short-term borrowings (Bn. VND)'] = 0
            short_term_col = 'Short-term borrowings (Bn. VND)'
        else:
            balance_sheet_copy[short_term_col] = balance_sheet_copy[short_term_col].fillna(0)
        ...
        balance_sheet_copy = balance_sheet_copy.rename(columns={
            long_term_col: 'Long-term borrowings (Bn. VND)',
            short_term_col: 'Short-term borrowings (Bn. VND)',
            equity_col: "OWNER'S EQUITY(Bn.VND)"
        })
