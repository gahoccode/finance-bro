...
def render_date_range_picker(
    ...
    if f"{key_prefix}_start_date" not in st.session_state:
        st.session_state[f"{key_prefix}_start_date"] = pd.to_datetime(
            DEFAULT_ANALYSIS_START_DATE
        )

    if f"{key_prefix}_end_date" not in st.session_state:
        st.session_state[f"{key_prefix}_end_date"] = pd.to_datetime(
            "today"
        ) - pd.Timedelta(days=1)
    ...
    if (
        start_date != st.session_state[f"{key_prefix}_start_date"]
        or end_date != st.session_state[f"{key_prefix}_end_date"]
    ):
...
def render_date_range_status(
    key_prefix: str = "analysis", show_details: bool = True
) -> None:
    """
...
def render_preset_date_ranges(
    key_prefix: str = "analysis", show_custom: bool = True
) -> None:
    """
...
def _update_date_range(
    key_prefix: str, start_date: datetime, end_date: datetime
) -> None:
    """
