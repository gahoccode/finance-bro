...
class FinancialAnalysisToolInput(BaseModel):
    ...
    analysis_type: str = Field(default="overview", description="Type of analysis to perform: 'overview', 'ratios', or 'trends'")


class FinancialAnalysisTool(BaseTool):
    name: str = "Financial Analysis Tool"
    description: str = "Access and analyze financial dataframes from Streamlit session state"
    ...
    def _run(self, analysis_type: str = "overview") -> str:
        ...
        try:
            ...
            if not dataframes:
                return "Financial dataframes are empty. Please load financial data first."
    ...
    def _get_overview_analysis(self, dataframes: Dict[str, pd.DataFrame]) -> str:
        ...
        for key, df in dataframes.items():
            if not df.empty and 'yearReport' in df.columns:
                ...
                current_year = max(years) if not current_year else max(current_year, max(years))
            ...
            if not df.empty:
                ...
                if current_year and 'yearReport' in df.columns:
                    ...
                    if not current_data.empty:
                        ...
                        key_items = self._identify_key_line_items_with_values(current_data, key)
                ...
                if len(sorted_years) > 1 and 'yearReport' in df.columns:
                    ...
                    trend_metrics = self._get_trend_analysis_for_statement(df, sorted_years, key)
                    analysis += trend_metrics
                
                # Data quality assessment
                numeric_cols = df.select_dtypes(include=['number']).columns
                non_zero_data = df[numeric_cols].replace(0, pd.NA).dropna(how='all', axis=1)
                analysis += f"\nDATA QUALITY:\n"
        ...
        if latest_data:
            ...
            health_indicators = self._calculate_financial_health_indicators(latest_data, current_year)
            analysis += health_indicators
        
        return analysis

    def _extract_key_metrics(self, df: pd.DataFrame, statement_type: str, analysis: str) -> None:
    ...
    def _identify_key_line_items_with_values(self, data: pd.DataFrame, statement_type: str) -> Dict[str, float]:
        ...
        for display_name, search_patterns in patterns.items():
            for pattern in search_patterns:
                matching_cols = [col for col in latest_row.index if pattern.lower() in col.lower()]
                if matching_cols:
    ...
    def _get_comprehensive_financial_context(self, dataframes: Dict[str, pd.DataFrame]) -> str:
            ...
            if current_year and 'yearReport' in df.columns:
                ...
                if not current_data.empty:
                    ...
                    for col in numeric_cols:
                        if col != 'yearReport' and pd.notna(latest_row[col]) and latest_row[col] != 0:
                            value = latest_row[col]
                            context += f"• {col}: {self._format_financial_number(value)}\n"
                        ...
                        if metric in df.columns:
                            ...
                            for year in sorted_years[-3:]:  # Last 3 years
                                ...
                                if not year_data.empty:
                                    value = year_data[metric].iloc[-1] if not pd.isna(year_data[metric].iloc[-1]) else 0
                    ...
                    for col in numeric_cols:
                        if any(pattern in col.lower() for pattern in ratio_patterns):
                            if pd.notna(latest_row[col]) and latest_row[col] != 0:
                                category_ratios.append(f"  • {col}: {latest_row[col]:.2f}")
        ...
        for statement_name, df in dataframes.items():
            ...
            for metric in key_metrics[:5]:  # Top 5 key metrics
                if metric in df.columns:
                    ...
                    for year in years:
                        year_data = df[df['yearReport'] == year]
                        if not year_data.empty and not pd.isna(year_data[metric].iloc[-1]):
                    ...
                    if len(trend_data) >= 2:
                        ...
                        if trend_data[-1][1] != 0 and trend_data[0][1] != 0:
                            growth_rate = ((trend_data[-1][1] / trend_data[0][1]) - 1) * 100
                ...
                for _, row in recent_data.iterrows():
                    ...
                    for col in numeric_cols:
                        if col != 'yearReport' and pd.notna(row[col]) and row[col] != 0:
                            context += f"  {col}: {self._format_financial_number(row[col])}\n"
            
            context += "\n" + "="*50 + "\n"
        
        return context
    
    def _get_key_metrics_for_statement(self, df: pd.DataFrame, statement_name: str) -> List[str]:
        """Get key metrics based on statement type"""
        all_cols = df.columns.tolist()
        
        if 'income' in statement_name.lower():
            patterns = ['revenue', 'sales', 'gross', 'operating', 'net_income', 'profit', 'ebit']
    ...
    def _calculate_financial_health_indicators(self, latest_data: Dict[str, pd.Series], current_year: int) -> str:
            ...
            if income_data is not None:
                ...
                if revenue_metrics:
                    ...
                    for metric, value in revenue_metrics.items():
                        indicators += f"• {metric}: {self._format_financial_number(value)}\n"
