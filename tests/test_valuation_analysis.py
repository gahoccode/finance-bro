...
class TestDataAlignment:
    ...
    def test_partial_date_overlap(self):
        ...
        stock_data = pd.DataFrame({
            'close': np.random.randn(len(stock_dates)) + 100
        }, index=stock_dates)
...
class TestBetaCalculation:
    ...
    def test_insufficient_data_points(self):
        ...
        small_dates = pd.date_range(start='2024-01-01', end='2024-01-10', freq='D')  # 10 days
        
        stock_data = pd.DataFrame({
            'close': np.random.randn(len(small_dates)) + 100
        }, index=small_dates)
...
class TestRiskClassification:
    ...
    @pytest.mark.parametrize("beta,expected_risk", [
...
class TestValuationIntegration:
    ...
    def test_complete_valuation_workflow(self, sample_stock_data, sample_vnindex_data, 
                                       sample_balance_sheet, sample_ratios):
        ...
        total_debt = (latest_balance_sheet.get('Short-term borrowings (Bn. VND)', 0) + 
        ...
        assert total_market_capital > 0
        assert abs(market_weight_of_debt + market_weight_of_equity - 1.0) < 1e-10  # Weights sum to 1
    ...
    def test_valuation_results_table_structure(self, sample_balance_sheet):
        ...
        assert '37.5%' in results_df.iloc[0]['Debt Weight']  # Check percentage formatting
