...
class TestConvertDataframeForDisplay:
    ...
    def test_convert_preserves_original_dataframe(self):
        ...
        convert_dataframe_for_display(self.test_df, columns_to_format, "billions", 1)

        # Original dataframe should be unchanged
        pd.testing.assert_series_equal(
            self.test_df["Long-term borrowings (Bn. VND)"], original_values
        )
    ...
    def test_convert_with_nan_values(self):
        ...
        df_with_nan.iloc[
            0, df_with_nan.columns.get_loc("Long-term borrowings (Bn. VND)")
        ] = pd.NA
    ...
    def test_convert_edge_case_zero_values(self):
        ...
        df_with_zeros.iloc[
            0, df_with_zeros.columns.get_loc("Long-term borrowings (Bn. VND)")
        ] = 0
...
class TestFinancialFormattingIntegration:
    ...
    def test_full_workflow_billions(self):
        ...
        revenue_formatted = format_financial_display(
            self.sample_data.iloc[0]["Revenue (Bn. VND)"], "billions", 1
        )
    ...
    def test_mixed_format_requirements(self):
        ...
        revenue_billions = format_financial_display(
            self.sample_data.iloc[0]["Revenue (Bn. VND)"], "billions", 2
        )

        revenue_original = format_financial_display(
            self.sample_data.iloc[0]["Revenue (Bn. VND)"], "original", 0
        )
