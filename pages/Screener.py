...
with col2:
    if st.button("🛡️ Low Risk Quality", help="High financial health > 7, Low beta < 1.2, High stock rating > 6"):
        # Clear all previous preset values
        for key in list(st.session_state.keys()):
            if key.startswith('preset_'):
                del st.session_state[key]
...
if use_beta:
    default_beta = (0.0, 1.2) if st.session_state.get('preset_beta', False) else (0.5, 2.0)
...
if use_financial_health:
    default_health = (7.0, 10.0) if st.session_state.get('preset_financial_health', False) else (5.0, 10.0)
...
if use_stock_rating:
    default_rating = (6.0, 10.0) if st.session_state.get('preset_stock_rating', False) else (5.0, 10.0)
...
if 'screener_data' in st.session_state and not st.session_state['screener_data'].empty:
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["ROE Analysis", "Market Cap vs Dividend Yield", "Value vs Quality", "Risk Analysis", "Quality Scores", "Distribution Charts"])
    ...
    with tab4:
        ...
        if 'beta' in df.columns and 'alpha' in df.columns:
            ...
            if not viz_df.empty:
                chart = create_scatter_chart(viz_df, 'beta', 'alpha', 'Risk Analysis: Beta vs Alpha')
                st.altair_chart(chart, use_container_width=True)
                st.caption("💡 **Sweet Spot**: Low Beta (<1.0) + High Alpha (>0) = Low risk with market outperformance")
            ...
            st.info("Risk analysis requires both Beta and Alpha data. These metrics may not be available in the current dataset.")
    ...
    with tab5:
        ...
        if 'financial_health' in df.columns and 'business_model' in df.columns:
            ...
            if not viz_df.empty:
                chart = create_scatter_chart(viz_df, 'financial_health', 'business_model', 'Quality Scores: Financial Health vs Business Model')
                st.altair_chart(chart, use_container_width=True)
                st.caption("💡 **Quality Leaders**: High Financial Health (>7) + High Business Model (>7) = Strong fundamentals")
                ...
                **Score Guidelines (0-10 scale):**
                ...
                st.info("No stocks have both Financial Health and Business Model scores available")
        else:
            st.info("Quality analysis requires both Financial Health and Business Model data. These metrics may not be available in the current dataset.")
    ...
    with tab6:
        ...
        with col1:
            ...
            if 'financial_health' in df.columns:
                ...
                if not viz_df.empty:
                    hist_chart = create_histogram(viz_df, 'financial_health', 'Financial Health Distribution')
                    st.altair_chart(hist_chart, use_container_width=True)
        ...
        with col2:
            ...
            if 'stock_rating' in df.columns:
                ...
                if not viz_df.empty:
                    hist_chart = create_histogram(viz_df, 'stock_rating', 'Stock Rating Distribution')
                    st.altair_chart(hist_chart, use_container_width=True)
    ...
    display_columns = ['ticker', 'exchange', 'industry', 'market_cap', 'roe', 'roa', 'dividend_yield', 'ev_ebitda', 'pe', 'pb', 
                      'beta', 'alpha', 'financial_health', 'business_model', 'business_operation', 'stock_rating']
    ...
    for col in numeric_cols:
        if col in display_df.columns:
            ...
            elif col in ['financial_health', 'business_model', 'business_operation', 'stock_rating']:
